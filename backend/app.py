...
def upload_image():
    ...
    while destination.exists():
        destination = UPLOAD_DIR / f"{Path(safe_name).stem}-{count}{Path(safe_name).suffix}"
        count += 1

    image.save(destination)
    image_url = request.host_url.rstrip("/") + f"/assets/uploads/{destination.name}"
...
def list_fabrics():
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT * FROM fabrics ORDER BY name")
    rows = cur.fetchall()
    cur.close()
    conn.close()

    fabrics = [row_to_dict(r) for r in rows]
...
def get_fabric(slug):
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT * FROM fabrics WHERE slug = %s", (safe_slug,))
    row = cur.fetchone()
    cur.close()
    conn.close()
    if not row:
...
def create_fabric():
    ...
    try:
        ...
        conn.commit()
        cur.execute("SELECT * FROM fabrics WHERE id = %s", (new_id,))
        row = cur.fetchone()
    except Exception:
        conn.rollback()
        cur.close()
...
def update_fabric(fabric_id):
    ...
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT * FROM fabrics WHERE id = %s", (fabric_id,))
    ...
    try:
        cur.execute(
            ...
            UPDATE fabrics SET
                ...
                sustainability = %s, reviews_count = %s
            WHERE id = %s
            """,
            (
                payload["slug"],
